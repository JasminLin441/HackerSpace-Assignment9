...
def recover_key_bit(a, b, S):
    count0 = 0  # count when a·x ⊕ b·y == 0
    count1 = 0  # count when a·x ⊕ b·y == 1
    ...
    bias0 = (count0 - len(pairs)/2) / len(pairs)
    bias1 = (count1 - len(pairs)/2) / len(pairs)
    # The sign of bias tells the value of b·K
    guessed_parity = 0 if bias0 > 0 else 1
    print(f"Approx {hex(a)}->{hex(b)}, bias0 ≈ {bias0:.3f},  bias1 = {bias1:.3f}, guessed b·K = {guessed_parity}")
    return guessed_parity
...
if __name__ == "__main__":
    ...
    recover_key_bit(0b0001, 0b0111, S)
